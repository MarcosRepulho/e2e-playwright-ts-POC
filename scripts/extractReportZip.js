...
(async ()=>{
  try{
    const html = fs.readFileSync(path.join(__dirname,'..','reports','html','index.html'),'utf8');
    ...
    function walk(dir){
      ...
      for(const name of fs.readdirSync(dir)){
        ...
        if(stat.isDirectory()) results.push(...walk(full)); else results.push(full);
      }
      return results;
    }
    ...
    const searchTerms = ['should not login with invalid credentials','should login with valid credentials','Login flow'];
